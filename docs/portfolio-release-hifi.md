...
At High Fidelity, I played a key role in crafting clear, developer-focused release notes for each release of the platform and SDK. I collaborated closely with engineers to document new features, improvements to existing features, and bug fixes.

Below is an example of the release notes from a High Fidelity release.
...
* **Blend shape support for ARKit compatibility**: We've added new blend shapes and add support for up to 10 custom blend shapes with this release. In addition, there are now Controller actions to drive all blend shapes from the input system. 
...
* In the Create Tool's Properties, increased the range for a zone's 'Haze Base' and 'Haze Ceiling' to (-16000 to 16000). Previously, this was capped to (-5000 to 5000). 
* Incorporated an experimental custom shader feature, which allows users to apply shaders to models and avatars. 

#### Scripting API

* Added the ``LODManager.worldDetailQuality`` type definition to define the level of world detail detail via scripting. Previously, the level of detail was set using a numerical range (0-1), which resulted in inconsistent/unexpected target FPS values. 
...
* When loading avatar bookmarks, avatar entities are now attached after the skeleton has finished loading. Previously, avatar entities may have erroneously been attached to incorrect joints due to old skeleton structures. 
